...
// Use local vendor path to avoid CDN/content-blocker issues. Change to CDN only if desired.
...
function loadExternalResource(url, type) {
	return new Promise((resolve, reject) => {
		...
		if (type === "css") {
			...
			// Load waifu-tips.js as an ES module (it contains `export`) to avoid
			// `Unexpected token 'export'` when parsing. Other scripts are loaded
			// as classic scripts so they can attach globals (e.g. live2d runtime).
			if (String(url).endsWith("waifu-tips.js")) {
				tag.type = "module";
				tag.src = url;
			} else {
				tag.src = url;
